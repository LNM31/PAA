...
void afisare(Pagina* biblio, int nivel)
{
	if (biblio == NULL)
		return;

	printf("Nivel %d: ", nivel);
	for (int i = 1; i <= biblio->m; i++)
	{
		printf("%s ", biblio->e[i].cheie);
	}
	printf("\n");

	afisare(biblio->p0, nivel + 1);
	for (int i = 1; i <= biblio->m; i++)
		afisare(biblio->e[i].p, nivel + 1);
}

void cautare(Pagina* biblio, const char* category)
{
	if (biblio == NULL)
		return;
	...
	{
		mij = (st + dr) / 2;
		if (strcmp(category, biblio->e[mij].cheie) == 0)
			afisare(biblio->e[mij].p,0);
		if (strcmp(category, biblio->e[mij].cheie) < 0)
			dr = mij - 1;
		else
			st = mij + 1;
	...
	if (dr == 0)
		return cautare(biblio->p0, category);
	else
		return cautare(biblio->e[dr].p, category);
}
void citireNod(Pagina** biblio, const char* file_name)
{
	...
	{
		*biblio = insereaza(*biblio, buffer, NULL);
	}
	if (fclose(f) != 0)
	{
...
{
	Pagina* biblio = NULL;
	citireNod(&biblio, "1_data_in.txt");
	afisare(biblio, 0);
	printf("\n");
	cautare(biblio,"Chemistry");
	return 0;
}